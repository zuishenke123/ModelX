...
def generate_test_cases():
    ...
    test_cases = []
    # 生成固定的卷积权重和偏置
    # 对于 nn.Conv3d，权重的形状为 [out_channels, in_channels, depth, height, width]
    weights = np.random.randn(6, 3, 3, 3, 3).astype(np.float32)  # 例如：6个输出通道，3个输入通道，3x3x3卷积核
    biases = np.random.randn(6).astype(np.float32)  # 偏置的形状为 [out_channels]
    for _ in range(10):
        # 输入数据的形状为 [batch_size, in_channels, depth, height, width]
        # 假设 depth, height 和 width 分别为 5
        input_array = np.random.randn(1, 3, 5, 5, 5).astype(np.float32)
...
def test_operator(test_cases):
    ...
    # 定义卷积层
    conv = nn.Conv3d(in_channels=3, out_channels=6, kernel_size=3, stride=1, padding=1)
